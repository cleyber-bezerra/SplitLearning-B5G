...
def process_file(file_path, color, label):
    ...
    # Calcular a média e erro padrão para rx_packets agrupados por Usere
    ...
    # Usar os valores reais de Users
    users_complete = np.arange(1, len(users) + 1)  # Ajustar para o tamanho correto
...
files = ['user_1.csv', 'user_2.csv', 'user_3.csv', 'user_4.csv', 'user_5.csv', 'user_6.csv']
colors = ['g', 'b', 'r', 'c', 'm', 'y']
labels = ['User 1 Accuracy', 'User 2 Accuracy', 'User 3 Accuracy', 'User 4 Accuracy', 'User 5 Accuracy', 'User 6 Accuracy']
...
plt.xticks(np.arange(1, 8), rotation=0, fontsize=13) # quantidade de usuarios
