...
int main(int argc, char* argv[])
{
    ...
    for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin(); i != stats.end(); ++i)
    {
        ...
        // Armazenar os valores nos vetores, assumindo que o índice corresponde ao UE
        lostPacketsVector[i->first] = lostPackets;
        throughputVector[i->first] = throughput;
        delayVector[i->first] = delay;
    ...
    size_t size = throughputVector.size() * sizeof(double) * 4; // 5 vetores (delay, throughput, consumo de energia, perda de pacotes, posicionamento)
    ...
    for (uint32_t i = 0; i < gridScenario.GetBaseStations().GetN(); ++i)
    {
        ...
        posFile << "gNB," << gNbNode->GetId() << "," << pos.x << "," << pos.y << "," << pos.z << "\n";
    }

    // Exportar posições dos UEs
    for (uint32_t i = 0; i < gridScenario.GetUserTerminals().GetN(); ++i)
    {
        Ptr<Node> ueNode = gridScenario.GetUserTerminals().Get(i);
        Ptr<MobilityModel> mob = ueNode->GetObject<MobilityModel>();
        Vector pos = mob->GetPosition();
        posFile << "UE," << ueNode->GetId() << "," << pos.x << "," << pos.y << "," << pos.z << "\n";
    }
    posFile.close();
