...
def process_file(file_path, color, label):
    ...
    grouped = df.groupby('User')['Percentage'].agg(['mean', 'std', 'count'])
...
files = ['accuracy.csv', 'delay.csv', 'lostPacketsVector.csv', 'throughput.csv', 'energyConsumption.csv']
colors = ['b', 'g', 'r','m','y']
labels = ['Accuracy', 'Delays', 'Packet Loss', 'Throughput', 'Energy Consumed']
...
plt.title('Accuracy Change', fontsize=16)
plt.xlabel('Distance from gNB (mt)', fontsize=15)
