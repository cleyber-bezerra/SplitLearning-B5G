...
def plot_ue_positions_with_distances(csv_file, output_file='ue_positions.png'):
    ...
    plt.colorbar(sc, label='Distânica do gNB (metros)')
    plt.xlabel('X Posição (metros)')
    plt.ylabel('Y Posição (metros)')
    plt.title('Posição dos dispositivos (UE) e distância do ponto de acesso (gNB)')
    plt.grid(True)
