...
def process_file(file_path, color, label):
    ...
    grouped = df.groupby('User')['EnergyConsumption'].agg(['mean', 'std', 'count'])
...
files = ['0/energyConsumption.csv', '13/energyConsumption.csv', '26/energyConsumption.csv']
colors = ['g', 'b', 'r']
...
plt.title('Canal de alta perda (4 dB)', fontsize=16)
plt.xlabel('Distância do gNB (mt)', fontsize=15)
plt.ylabel('Consumo Energético (J)', fontsize=15)
plt.grid(True)
...
name_output_file = "EnergyConsumptionH"
